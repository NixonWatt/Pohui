    ...
    var angle = 0,
        ...
        process = function (j) {
            var value = values[j],
                ...
                txt = paper.text(cx + (r + delta +50) * Math.cos(-popangle * rad), cy + (r + delta + 25) * Math.sin(-popangle * rad), labels[j]).attr({ fill: bcolor, stroke: "none", opacity: 0, "font-family": 'Fontin-Sans, Arial', "font-size": "20px" });
...
(function (raphael) {
    $(function () {
        ...
        $("table").hide();
        raphael("holder",600, 450).pieChart(200, 150, 100, values, labels, "#fff");
    });
})(Raphael.ninja());